    ...
    if(gotInfo == bigBlocks.end()){
        ...
        for(unsigned int i = 0; i < inUsePageNumbers.size(); i++){
            ...
            if(it != inUsePageNumbers[i].end()){
                ...
                if(memoryPages[pageNumber].GetMemory() + blockNumber * possibleBlockSizes[i] != addr){
                    return NULL;
                }
...
string PageMemoryAllocator::mem_dump()
{
    ...
    ret.append("Пам'яті всього (байт): ");
    ret.append(to_string(memorySize));
    ret.append("\nРозмір сторінки (байт): ");
...
{
    return NULL != addr && addr >= memory && addr <= memory + memorySize;
}
